...
the hand-drawn SVGs are Plates 02–04 (fleet sensitivity, robustness, fleet
mix). This module holds the shared parts so the plates cannot drift apart:

* **Surface & ink.** A near-white board (``#fcfcfb``), near-black primary ink,
  a secondary and a muted ink for supporting text, hairline gridlines and a
  slightly darker baseline/axis tone. Text always wears ink, never a series
  colour.
* **A validated categorical palette** for per-vehicle route colours — eight
  fixed slots in a fixed order, checked with the data-viz palette validator
  (adjacent-pair CVD ΔE >= 8, normal-vision ΔE >= 15, lightness band, chroma
  floor; light surface ``#fcfcfb``). Slots are assigned by vehicle number and
  **never cycled**: vehicles 9–16 reuse slots 1–8 with a dashed line — a
  composite colour+dash encoding — and every route also carries a direct
  ``V<n>`` label, so identity is never colour-alone. Three slots (aqua,
  yellow, magenta) sit below 3:1 contrast on the light surface; the relief is
  that every plate ships direct labels and a CSV/MD table of the same numbers.
* **Semantic series roles.** Across plates, blue is the primary cost measure
  ...
  aqua the emissions measure, and ``REF_GRAY`` a deliberately de-emphasised
  reference series (a baseline drawn as quiet gray under the inked answer —
  emphasis, not a second rainbow). The gray is *meant* to read gray; its
  separation from blue was still checked (CVD ΔE 15.9, normal 17.8) and both
  lines are directly labelled.
...
* **Stacked money bars** use a one-hue ordinal blue ramp (light ``#86b6ef``
  fixed cost under deep ``#2a78d6`` variable cost — validator ``--ordinal``
  pass), with a 2px surface gap between segments and the total labelled.
...
# --- board surface & ink (light mode; the deliverables are light-only) -------
SURFACE = "#fcfcfb"  # the board itself
INK = "#0b0b0b"  # primary ink: titles, key figures
SECONDARY = "#52514e"  # supporting text, direct labels
MUTED = "#898781"  # axis tick labels, eyebrows, captions
GRID = "#e1e0d9"  # hairline gridlines
AXIS = "#c3c2b7"  # baseline / axis / frame lines
...
BLUE = VEHICLE_SLOTS[0]  # primary cost measure (distance km, EUR)
ORANGE = VEHICLE_SLOTS[1]  # service / risk measure
AQUA = VEHICLE_SLOTS[2]  # emissions measure
BLUE_LIGHT = "#86b6ef"  # fixed-cost segment of the money stack (ordinal ramp)
REF_GRAY = "#898781"  # de-emphasised reference series (deliberate gray)
BASELINE_GRAY = "#c9c8c1"  # map underlay for the heuristic baseline routes
